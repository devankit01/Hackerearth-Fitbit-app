...
def signup(request):
    if request.method == 'POST':
        ...
        print(first_name,last_name,email,pass1,pass2,weight,height,age,excercise,gender)
        if pass1 == pass2:
            try:
                email = request.POST['email']
                ...
                if gender:
                    BMR  = 66.47 + (13.75 * int(weight)) + (5.003 * int(height)) - (6.755 * int(age ))
                ...
                new_user = User.objects.create_user(first_name = first_name , last_name = last_name , email = email , username = email , password = pass1)
                new_user.save(False)
                new_pro = Profile(username = new_user , age = age , height = height , weight = weight , profile_img ="" , excercise_status = excercise , calorie_needed = BMR)
                new_user.save()
                new_pro.save()
...
def check(request,id,user):
    ...
    if kk.action==False:
        kk.action=True
        kk.save()
    total_task=personTask.objects.filter(user=user,date=date.today(),action=True)
    print(total_task)
...
def get_data(request):
    if request.method=="POST":
        ...
        obj=personTask.objects.create(user=main,task=name,date=date,time=time)
